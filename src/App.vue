...
// 打了關鍵字後，一鍵把目前符合搜尋的所有選項加入已選（與現有選取做聯集）
function selectAllMatching(optsKey, key) {
  const matching = filteredOptions(filterOptions.value[optsKey], key).map(o => o.val)
...
function dualAxisBarSvg(chartData) {
  ...
  // 軸最大值依「實際最高的長條」(各組各分段的數值)決定，會隨篩選後資料自動調整
  ...
  <div class="dashboard" :class="{ 'is-loading': loading }">
                  ...
                  <div v-if="openAcc === item.key" class="acc-body">
                    <div class="filter-label-row">
                      ...
                      <div style="display:flex;align-items:center;gap:4px">
                        ...
                        <button v-if="filterOptions[item.optsKey].length > 10 && getSearch(item.key).trim()" @click="selectAllMatching(item.optsKey, item.key)" class="select-all-btn" title="全選目前符合搜尋的項目">全選</button>
    ...
    <div :class="['main-area', 'bg-' + activeCat]">
      <div class="main-header">
        <h1 style="font-size:22px;font-weight:700;margin:0;color:#0f172a">裁判書量化實證研究平台－民國113-114年</h1>
        ...
        <template v-for="ch in chartLayout" :key="ch.key">
          ...
          <div class="chart-card" v-if="ch.type === 'lawyerEndingAndAmount'">
            ...
            <div style="margin-top:24px;padding-top:20px;border-top:1px dashed #cbd5e1" v-if="charts.amountLawyerHeatmap">
              <div class="chart-title" style="font-size:15px">標的金額 × 律師代理</div>
              <div class="chart-sub" style="font-size:12px;margin-bottom:12px">標的金額級距與律師代理情形之交叉分析</div>
              ...
              <div v-if="charts.initiatorDist?.length" style="margin-top:16px;padding-top:12px;border-top:1px solid #e5e7eb">
                ...
                <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">
                  <svg width="140" height="140" viewBox="0 0 140 140">
                    ...
                    <text x="70" y="67" text-anchor="middle" font-size="10" fill="#374151" font-weight="700"></text>
                    <text x="70" y="80" text-anchor="middle" font-size="10" fill="#374151" font-weight="700"></text>
                  </svg>
                  <div>
...
.select-all-btn { padding: 2px 7px; border-radius: 8px; border: 1px solid #2563eb; background: #eff6ff; color: #2563eb; font-size: 10px; font-family: inherit; font-weight: 600; cursor: pointer; white-space: nowrap; }
.select-all-btn:hover { background: #2563eb; color: #fff; }
